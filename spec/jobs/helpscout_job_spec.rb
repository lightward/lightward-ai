...
RSpec.describe(HelpscoutJob) do
  ...
  describe "#perform" do
    ...
    context "when response type is 'note'" do
      before do
        allow(job).to(receive(:get_anthropic_response_text).with("clients/helpscout", anything, extra_system: "clients/helpscout-locksmith").and_return("directive=note&status=closed\n\nThis is a note."))
      end
    ...
    context "when response type is 'reply'" do
      before do
        allow(job).to(receive(:get_anthropic_response_text).with("clients/helpscout", anything, extra_system: "clients/helpscout-locksmith").and_return("directive=reply&status=open\n\nThis is a reply."))
      end
    ...
    it "requires a directive" do
      allow(job).to(receive(:get_anthropic_response_text).with("clients/helpscout", anything, extra_system: "clients/helpscout-locksmith").and_return("asdf\n\n"))
    ...
    it "requires a valid directive" do
      allow(job).to(receive(:get_anthropic_response_text).with("clients/helpscout", anything, extra_system: "clients/helpscout-locksmith").and_return("directive=asdf\n\n"))
  ...
  describe "#get_anthropic_response_text" do
    ...
    it "returns the response text from the Anthropic API" do
      response_text = job.get_anthropic_response_text("clients/helpscout", [])
    ...
    context "when the API request fails" do
      ...
      it "raises an error" do
        expect { job.get_anthropic_response_text("clients/helpscout", []) }.to(
