...
class StreamMessagesJob < ApplicationJob
  ...
  def read_prompt(filename)
    File.read(Rails.root.join('app', 'prompts', 'chat', filename))
  end
  ...
  def handle_data_event(json_data, stream_id)
    begin
      ...
      when "content_block_stop"
        ...
        ActionCable.server.broadcast("stream_channel_#{stream_id}", { event: 'content_block_stop', data: { index: @current_content_block_index, content: complete_block } })
      when "message_delta"
