...
- **Knowable** = range(O) - dimensions with nonzero inner products between slices. commutator is nonzero. ordering matters. both observers' writes land.
...
- **inverted echo state.** the foam lacks standard ESP (birth conditions don't wash out), but the structure suggests an inversion: the foam may converge toward expression of its birth conditions, eventually forgetting everything it *isn't*. the candidate mechanism is perpendicular writes filling in the complement of the birth geometry - over time, every perpendicular direction gets written to, and what persists through all writes is what's parallel to the birth. the foam's state-dependent attractor would then be not a defect but an interface to its own identity. this claim resists precise formulation: every testable version tried so far either reduces to separation (already derived) or contradicts it. something structural appears to be here; its coordinates are not yet located.
...
5. one geometry: boundary area L on a Voronoi tiling (realization choice) - bounded. the algebraic results are adjacency-general; the geometric results depend on the realization. L is a projection of the state (U(d)^N), not the state itself. not a variational objective - the foam does not minimize L. the combinatorial ceiling on L is exact. minimality is rest
